...
def simulator_wrapper(kwargs):
    ...
    print(results)
    with open("results/pool/10_disturbance/" + str(uuid.uuid4()) + ".pkl", "wb") as tf:
...
if __name__ == "__main__":
    ...
    for packet_type in packet_type_list:
        for network_info in network_info_list:
            ...
            disturbance_num_list = network_info["disturbance_num_list"]
            capacity_conditions = network_info["capacity_conditions"]
            for waiting_cost in waiting_cost_list:
                for disturbance_num in disturbance_num_list:
                    for capacity_condition in capacity_conditions:
                        for num_packets in num_packets_list:
                            memo = "disturbance " + packet_type + str(num_packets) + " " + network_type + " " + capacity_condition[
                            ...
                            condition = {
                                ...
                                "capacity_type": capacity_condition["capacity_type"], "capacity_coef": capacity_condition["capacity_coef"],
                                "with_disturbance": True, "disturbance_type": "random",
                                "fixed_disturbance": [], "disturbance_length": 5, "disturbance_num": disturbance_num,
                                "generate_pattern": None, "is_traced": False, "packet_size": 2,
                                "generate_pattern_trace": None, "T_max": T_max, "condition_memo": memo
                            }
